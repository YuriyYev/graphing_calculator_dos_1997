...
- **26 variables** — each can hold a number or an expression referencing other variables, with a per-variable auto-recalculate flag for iterative computation
- **Full math library** — trig (sin, cos, tan, ctan and inverses), hyperbolic functions, roots of degrees 1–9, powers, logarithms (ln, lg, log base N), factorial, sign, and more
- **4 number systems** — decimal, binary, octal, hexadecimal; switchable per-expression with per-token number base
- **Angle measure per token** — each trig function in an expression can independently use radians, degrees, or gradians
- **Summation operator Σ** — iterates `i` from 0 to `n-1`, enabling numerical integration in a single expression
- **Statistical buffer** — up to 111 values; supports sum, mean, and other statistical operations over the set
...
No libc. No graphics library. No math library. No input library. Everything is implemented from scratch:

| What | How |
|------|-----|
| Math functions | x87 FPU instructions (`FSIN`, `FCOS`, `FSQRT`, `FYL2X`, `FPATAN`, `F2XM1`...) |
| Number → string | `FBSTP` — FPU stores directly as packed BCD, 18 digits in 9 bytes |
...
The Σ operator makes numerical integration possible in a single expression. To approximate ∫x² dx from A to B:

```
Sum((i×(B-A)/(n-1) + A)² × (B-A)/n)
```

With `A=0`, `B=5`, `n=100000`: result is `41.666875` (exact: `125/3 = 41.6̄`).
...
Written by Yuriy in Ukraine, starting around 1997, at age 16. A longer writeup of how it was built — the parser, the FPU tricks, the DAC fade effect, the mouse interrupt architecture — is available as an article: *[link to article]*

---

*YURA'S PRODUCTION © 1997*
